...
class WaveBrush {
  constructor(xPos, yPos, width, height, red, green, blue) {
    ...
    //setup the size
    this.size = 1;
    this.noiseOff1 = random(1000); // Offset for Perlin noise in x direction
    this.noiseOff2 = random(1000); // Offset for Perlin noise in y direction
  ...
  update(size, velocity, red, green, blue) {
    ...
    this.red = red + map(noise(this.noiseOff1), 0, 1, -30, 30)
    this.green = green + map(noise(this.noiseOff1), 0, 1, -30, 30)
    this.blue = blue + map(noise(this.noiseOff2), 0, 1, -30, 30)
   // this.color.setAlpha(alpha)
    this.size = size * 10 + map(noise(this.noiseOff1), 0, 1, 0, 30);
    ...
    this.noiseOff1 += 0.005;
    this.noiseOff2 += 0.005;
  }
